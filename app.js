...
let studyURL = "";
let sectionsRaw = [];
let partOneDisclaimer = "";
let partTwoTitle = "";
let consentDescription = "";
function populatePlaceholder(){
    formTitle = "Informed Consent Form";
    partOneTitle = "Part I. Information Sheet";
    studyURL = "https://en.wikipedia.org/wiki/Melange_(fictional_drug)";
    sectionsRaw = [
        { 
    ...
    partOneDisclaimer = "This proposal has been reviewed and approved by the Parmentier Medical Review Board, which is a committee whose task it is to make sure that research participants are protected from harm. If you wish to find out more about the PMRB, visit their website. It has also been reviewed by the Ethics Review Committee of the Landsraad Health Organization (LHO), which is sponsoring the study.";
    partTwoTitle = "Part II. Ceritificate of Consent";
    consentDescription = "I have read the information on this page, or it has been read to me. I have had the opportunity to ask questions about it and any questions that I have asked have been answered to my satisfaction. I consent voluntarily to participate as a participant in this research.";
};

// Trying out the editing tool
let editedSectionsRaw = [];

// Universal sections bucket
let sections = [];
...
app.get("/edited", function(req, res){
    ...
    let annotationCount = 0;
    // Add annotations
    for (let index = 0; index < editedSectionsRaw.length; index++) {
        let bodyText = editedSectionsRaw[index].body;
        for (let i = 0; i < bodyText.length; i++){
            bodyText = bodyText.replace("{", "<a id='" + annotationCount + "' class='annotation annotation-link'>");
            bodyText = bodyText.replace("}", "</a>");
            bodyText = bodyText.replace("[", "<span id='annotation" + annotationCount + "' class='annotation annotation-body' style='display: none'>");
        ...
        const section = {
            header: editedSectionsRaw[index].header,
            body: bodyText,
        }
        sections.push(section);
    }
...
app.post("/edit", function(req,res){
    ...
    editedSectionsRaw.push(about, intro, purposeResearch, typeOfResearch, selection, voluntPart, infoOnDrug, protocol, descOfProcess, duration, sideEffects, risks, benefits, reimbursements, confidentiality, sharing, rightToRefuse, alts, contact);
